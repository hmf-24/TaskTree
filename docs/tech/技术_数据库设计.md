...
author: HMF
created: 2026-04-10
description: "Nexus 数据库设计 v2.0，包含全局用户与 TaskTree/ReadHub 等核心模块的表结构设计"
tags:
  - Status/Done
  - Type/Work/SOP
  - Area/Lab
...
## ReadHub 模块表结构 (Nexus 新增)

### 12. rss_feeds - RSS 订阅源表
...
| title | VARCHAR | 订阅源名称 |
| url | VARCHAR | 原始链接 |
| feed_url | VARCHAR | RSS 链接 |
| description | TEXT | 描述 |
| auto_summary | BOOLEAN | 是否开启 AI 自动摘要 |

### 13. rss_articles - 资讯文章表
...
| link | VARCHAR | 原文链接 |
| author | VARCHAR | 作者 |
| content | TEXT | 文章正文（提取后） |
| ai_summary | TEXT | AI 生成的摘要 |

### 14. rss_articles_fts - FTS5 虚拟表

通过 SQLite 原生触发器与 `rss_articles` 同步，提供毫秒级全文检索：
- 同步字段：`title`, `content`, `author`, `ai_summary`
- 实现方案：SQLite FTS5 + Triggers
