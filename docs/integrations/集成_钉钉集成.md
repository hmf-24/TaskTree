...
> [!info] 概述
> Nexus 生态下的智能终端已经深度集成了钉钉 Stream 机器人。目前支持**双重机器人隔离架构**：
> - **TaskTree 任务助手**：负责解析自然语言更新任务进度、分配任务、智能提醒。
> - **ReadHub 订阅助手**：负责接收公众号文章、生成按需摘要、指令搜索历史资讯。
> 
> *核心机制：通过钉钉应用的 App ID 与代码中的 `app_source` (tasktree / readhub) 进行隔离。*
...
在 `.env` 文件中，针对不同的 `app_source`，你可以配置多个应用参数（以 Stream 模式的 Client ID / Secret 为例，见 `.env.example`）：

```bash
# TaskTree 钉钉配置 (app_source="tasktree")
DINGTALK_CLIENT_ID_TASKTREE=your_client_id_for_tasktree
DINGTALK_CLIENT_SECRET_TASKTREE=your_secret_for_tasktree

# ReadHub 钉钉配置 (app_source="readhub")
DINGTALK_CLIENT_ID_READHUB=your_client_id_for_readhub
DINGTALK_CLIENT_SECRET_READHUB=your_secret_for_readhub
```
